<?php
    ...
    $select_active_status = "SELECT * FROM services WHERE features_status = 1";
    ...
    if($after_assoc['features_status'] == 1) {
        $update_active = "UPDATE services SET features_status = 0 WHERE id = '$id'";
        $update_active_result = mysqli_query($dbconnect, $update_active);
        header('location:view_service.php');
    }
    else {

        if($select_active_row > 2) {
            $_SESSION['error'] = 'Maximum 3 ta Dekhaite parben';
            header('location:view_service.php');
        }
        else {
            $update_active = "UPDATE services SET features_status = 1 WHERE id = '$id'";
            $update_active_result = mysqli_query($dbconnect, $update_active);
            header('location:view_service.php');
        }
    }
